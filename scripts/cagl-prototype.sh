...
echo "Grid:          2x2 centered, 540x730 each cell"
echo "Dead zones:    top 180px + bottom 280px"
echo "Safe zone:     1080x1460 (76% of canvas)"
echo "Optimized:     Stories-first (Reels has slight M3/M4 bottom edge overlap)"
echo ""
...
if [ ! -f "$SAMPLES_DIR/m1.mp4" ]; then
  echo "[1/3] Generating 4 sample test clips (540x730, 6s each)..."
  ...
  for i in 0 1 2 3; do
    ...
    ffmpeg -y -loglevel error \
      -f lavfi -i "color=c=${COLORS[$i]}:s=540x730:d=6:r=30" \
      -vf "drawtext=text='${LABELS[$i]}':fontsize=140:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2:fontfile=$FONT,drawtext=text='cell %{n}':fontsize=32:fontcolor=white@0.7:x=(w-text_w)/2:y=h-60:fontfile=$FONT" \
...
ffmpeg -y -loglevel warning \
  ...
  -filter_complex "
    [0:v]scale=540:730,setsar=1[v0];
    [1:v]scale=540:730,setsar=1[v1];
    [2:v]scale=540:730,setsar=1[v2];
    [3:v]scale=540:730,setsar=1[v3];
    [v0][v1][v2][v3]xstack=inputs=4:layout=0_0|w0_0|0_h0|w0_h0[grid];
    [grid]pad=1080:1920:0:180:color=#0A0A0A[padded];
    [padded]drawtext=text='vertical-log':fontsize=40:x=(w-text_w)/2:y=60:fontcolor=#DDDDDD:fontfile=$FONT[t1];
    [t1]drawtext=text='2026-04-26':fontsize=26:x=(w-text_w)/2:y=110:fontcolor=#888888:fontfile=$FONT[t2];
    [t2]drawtext=text='vertical-log.app':fontsize=26:x=(w-text_w)/2:y=1770:fontcolor=#888888:fontfile=$FONT[final]
  " \
  -map "[final]" \
  -t 6 \
